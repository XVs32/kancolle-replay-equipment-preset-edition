	...
	var UI_FLEETEDITOR = Vue.createApp({
		...
		methods: {
			...
			onchangeFileKC3: function (e) {
				...
				//you don't call every read func one by one here, 
			...
			readJsonFile: function (file) {
				let isDone = [];
				let jsonData = {};
				isDone.push(new Promise((resolve, reject) => {
					const reader = new FileReader();
					reader.onload = function (e) {
						try {
							jsonData[file.name] = JSON.parse(e.target.result);
							resolve(jsonData);
						} catch (error) {
							console.error(`Error parsing JSON content of ${file.name}:`, error);
							reject(error);
						}
					}
